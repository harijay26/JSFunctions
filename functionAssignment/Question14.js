...
const compose = (...func) => args => func.reduceRight((arg, fn) => fn(arg), args);


// I added trace() to this algorithm to inspect the values between functions for clarity!
trace = label => value => { // used curried method on label and value (can be written; trace(label,value))
  console.log(`${ label }: ${ value }`); // template litrals to display the outputs
  return value;
};
